<%@ page contentType="text/html;charset=UTF-8" language="java" %>
    <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        ...
        <head>
            ...
            <style>
                .mode-card { transition: all 0.3s ease; cursor: pointer; border: 2px solid transparent; }
                .mode-card:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(0,0,0,0.1); }
                .mode-card.active { border-color: #ae1a21; background: linear-gradient(135deg, #fff5f5 0%, #fff 100%); }
                .mode-card.active .mode-icon { background: #ae1a21; color: white; }
                .tab-btn { transition: all 0.2s ease; }
                .tab-btn.active { color: #ae1a21; border-bottom: 3px solid #ae1a21; font-weight: 600; }
                .status-badge { font-size: 11px; padding: 3px 10px; border-radius: 9999px; font-weight: 600; }
                .status-approved { background: #dcfce7; color: #166534; }
                .status-pending { background: #fef9c3; color: #854d0e; }
                .status-rejected { background: #fee2e2; color: #991b1b; }
                .ai-status-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
                .ai-online { background: #22c55e; box-shadow: 0 0 6px #22c55e; }
                .ai-offline { background: #ef4444; box-shadow: 0 0 6px #ef4444; }
                .fade-in { animation: fadeIn 0.3s ease; }
                @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
                .pulse-glow { animation: pulseGlow 2s infinite; }
                @keyframes pulseGlow { 0%, 100% { box-shadow: 0 0 5px rgba(174,26,33,0.3); } 50% { box-shadow: 0 0 15px rgba(174,26,33,0.6); } }
            </style>
        </head>
        ...
        <body class="bg-gray-100 font-sans text-gray-900"
            ...
            <main class="pt-20 pb-10">
                <div class="max-w-6xl mx-auto px-4 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-5 fade-in">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="flex items-center gap-2 text-xs">
                                ...
                                <button onclick="checkAiService()" class="text-gray-400 hover:text-primary transition ml-1" title="Kiểm tra lại">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="mode-card rounded-xl p-4 bg-gray-50 ${moderationMode == 'NONE' ? 'active' : ''}"
                                ...
                                <div class="mode-icon w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${moderationMode == 'NONE' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}">
                                    <i class="fas fa-unlock text-lg"></i>
                                </div>
                                <h3 class="font-semibold text-sm text-gray-900 mb-1">Không kiểm duyệt</h3>
                                <p class="text-xs text-gray-500">Bài viết hiển thị ngay khi đăng. Không qua bất kỳ bước kiểm tra nào.</p>
                            </div>
                            ...
                            <div class="mode-card rounded-xl p-4 bg-gray-50 ${moderationMode == 'MANUAL' ? 'active' : ''}"
                                ...
                                <div class="mode-icon w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${moderationMode == 'MANUAL' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}">
                                    <i class="fas fa-user-shield text-lg"></i>
                                </div>
                                <h3 class="font-semibold text-sm text-gray-900 mb-1">Kiểm duyệt thủ công</h3>
                                <p class="text-xs text-gray-500">Mọi bài viết phải chờ Admin xét duyệt trước khi hiển thị.</p>
                            </div>
                            ...
                            <div class="mode-card rounded-xl p-4 bg-gray-50 ${moderationMode == 'AUTO_AI' ? 'active' : ''}"
                                ...
                                <div class="mode-icon w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${moderationMode == 'AUTO_AI' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}">
                                    <i class="fas fa-brain text-lg"></i>
                                </div>
                                <h3 class="font-semibold text-sm text-gray-900 mb-1">Tự động (AI)</h3>
                                <p class="text-xs text-gray-500">AI PhoBERT tự động phân loại. Clean → duyệt, Toxic → từ chối.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-5 mt-6">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="font-semibold text-gray-900"><i class="fas fa-file-alt text-primary mr-2"></i>Quản lý Bài viết</h2>
                        ...
                        <div class="flex border-b border-gray-100 mb-4 gap-1">
                            <button class="tab-btn active px-4 py-2.5 text-sm text-gray-500" onclick="switchTab('all', this)">
                                Tất cả <span class="ml-1 text-xs bg-gray-100 px-2 py-0.5 rounded-full">${posts.size()}</span>
                            </button>
                            <button class="tab-btn px-4 py-2.5 text-sm text-gray-500" onclick="switchTab('pending', this)">
                                <i class="fas fa-hourglass-half text-yellow-500 mr-1"></i>Chờ duyệt
                                <span class="ml-1 text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full" id="tabPendingCount">${pendingCount}</span>
                            </button>
                            <button class="tab-btn px-4 py-2.5 text-sm text-gray-500" onclick="switchTab('rejected', this)">
                                <i class="fas fa-ban text-red-500 mr-1"></i>Bị từ chối
                                <span class="ml-1 text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">${rejectedCount}</span>
                            </button>
                            <button class="tab-btn px-4 py-2.5 text-sm text-gray-500" onclick="switchTab('approved', this)">
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-100">
                                        ...
                                        <th class="text-left text-xs font-semibold text-gray-400 pb-3 pr-4">Trạng thái</th>
                                        <th class="text-left text-xs font-semibold text-gray-400 pb-3 pr-4">AI Label</th>
                                            ...
                                            <td class="py-3 pr-4">
                                                <c:choose>
                                                    <c:when test="${p.status == 'PENDING'}">
                                                        <span class="status-badge status-pending"><i class="fas fa-hourglass-half mr-1"></i>Chờ duyệt</span>
                                                    </c:when>
                                                    <c:when test="${p.status == 'REJECTED'}">
                                                        <span class="status-badge status-rejected"><i class="fas fa-ban mr-1"></i>Từ chối</span>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <span class="status-badge status-approved"><i class="fas fa-check-circle mr-1"></i>Đã duyệt</span>
                                                    </c:otherwise>
                                                </c:choose>
                                            </td>
                                                ...
                                                <c:if test="${not empty p.moderationLabel}">
                                                    <c:choose>
                                                        <c:when test="${p.moderationLabel == 'CLEAN'}">
                                                            <span class="text-green-600 font-medium text-xs"><i class="fas fa-leaf mr-1"></i>Clean</span>
                                                        </c:when>
                                                        <c:when test="${p.moderationLabel == 'OFFENSIVE'}">
                                                            <span class="text-orange-500 font-medium text-xs"><i class="fas fa-exclamation-triangle mr-1"></i>Offensive</span>
                                                        </c:when>
                                                        <c:when test="${p.moderationLabel == 'HATE'}">
                                                            <span class="text-red-600 font-medium text-xs"><i class="fas fa-skull-crossbones mr-1"></i>Hate</span>
                                                        </c:when>
                                                    </c:choose>
                                                    <c:if test="${not empty p.moderationConfidence}">
                                                        <span class="text-gray-400 text-xs ml-1">${String.format("%.0f", p.moderationConfidence * 100)}%</span>
                                                    </c:if>
                                                </c:if>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-5 mt-6">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="font-semibold text-gray-900"><i class="fas fa-bullhorn text-primary mr-2"></i>Quản lý Thông báo</h2>
                            ...
                            <c:forEach var="ann" items="${announcements}">
                                <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-xl" id="ann-row-${ann.id}">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-gray-400 mt-1">${ann.createdAt} — ${ann.admin.fullName}</p>
                                    </div>
            ...
            <div id="addAnnouncementModal"
                class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
                    <h3 class="font-bold text-gray-900 text-lg mb-4"><i class="fas fa-bullhorn text-primary mr-2"></i>Tạo thông báo</h3>
                    <div class="space-y-3">
